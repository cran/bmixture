...
void update_z_t( int z[], int n_i[], int *df_t, 
			...
			)
{
	...
	for( int j = 0; j < *n_c; j++ )
	{
		// prob_z = pi * dt( ( data[j] - mu ) / sqrt( sig ), df_t )
		for( int i = 0; i < *k_c; i++ ) 
			prob_z[i] = pi_c[i] * Rf_dt( ( data_c[j] - mu_c[i] ) / sqrt( sig_c[i] ), *df_t, 0 ); // to check ??
			...
			double mu_c[], double sig_c[] )
{
	...
	for( int j = 0; j < *n_c; j++ )
	{
		...
		double df_post = 0.0;
		for( int i = 0; i < *k_c; i++ )
			df_post += z[ j * *k_c + i ] *  ( ( pow( ( data_c[j] - mu_c[i] ), 2.0 ) ) / sig_c[i] );
...
// update mu
void update_mu_t( double q_t[],
			double data_c[], int z[], int *n_c, int *k_c, 
			...
			)
{
	...
	for( int i = 0; i < *k_c; i++ ) 
	{
		...
		// sum_q_data = sum( z[i,] * q_t * data )
		double sum_q_data = 0.0;
		for( int j = 0; j < *n_c; j++ )
			sum_q_data += z[ j * *k_c + i ] * q_t[j] * data_c[j];
			
		// sum_q_t = sum( z[i,] * q_t )
		double sum_q_t = 0.0;
		for( int j = 0; j < *n_c; j++ )
			sum_q_t += z[ j * *k_c + i ] * q_t[j];
		
		// Set posterior mean and standard deviation
		// mu_sig  = 1 / ( n_i[i] / sig[i] + kappa_r ) 
		// mu_sig  = 1 / ( sum_q_t / sig[i] + kappa ) 
		double mu_sig  = 1.0 / ( sum_q_t / sig_c[i] + *kappa_c ); 
		//     mu_mean = ( sum_q_data / sig[i] + kappa * epsilon ) * mu_sig
		double mu_mean = ( sum_q_data / sig_c[i] + *kappa_c * *epsilon_c ) * mu_sig;
...
// update sig
void update_sig_t( double *beta_new, double q_t[],
			...
			)
{
	...
	for( int i = 0; i < *k_c; i++ ) 
	{
		...
		for( int j = 0; j < *n_c; j++ ) 
		{
			...
			double z_datamu = z[ j * *k_c + i ] * q_t[j] * pow( data_mu, 2.0 );
...
//-- STEP 1: sample latend variables (matrix z) -------------------------------|
	...
	vector<int> n_i( *k_c );
	update_z_t( &z[0], &n_i[0], df_t, data_c, n_c, k_c, mu_c, sig_c, pi_c );

//-- STEP 2: updating q_t ----------------------------------------------------|

	update_q_t( q_t, data_c, &z[0], df_t, n_c, k_c, mu_c, sig_c );
   
//-- STEP 3: updating beta ----------------------------------------------------|
   
	double beta_new;
	update_beta_t( &beta_new, n_c, k_c, alpha_c, g_c, h_c, sig_c );

//-- STEP 4: updating pi_c ----------------------------------------------------|
   			
	update_pi_t( pi_c, &n_i[0], n_c, k_c );
		
//-- STEP 5: updating mu_c ----------------------------------------------------|
   	
	update_mu_t( q_t, data_c, &z[0], n_c, k_c, epsilon_c, kappa_c, mu_c, sig_c	);
...
void sort_sample_t( double mu_c[], double sig_c[], double pi_c[], int *k_c )
{
	if( *k_c > 1 )   
	{ 		
...
*/
void bmix_t_unknown_k( double data_r[], int *n, int *k, int *k_max_r, int *iter, int *burnin, double *lambda_r,
...
// ------------  Birth-death step ---------------------------------------------|
		...
		{ 		
			for( j = 0; j < k_c; j++ )
			{
				...
				for( int t = 0; t < n_c; t++ )
				{
					// likelhood = sum( pi *   dnorm( data[t], mu, sqrt( sig / q_t[t] ) ) )
					double likelhood = 0.0;
					for( i = 0; i < k_c; i++ ) 
						likelhood += pi_c[i] * dnorm( data_c[t], mu_c[i], sqrt( sig_c[i] / q_t[t] ), 0 ); 
					
					if( likelhood == 0 ) likelhood = 1e-300; 
					
					//d_data_t_j = dnorm( data[t], mu[j], sqrt( sig[j] ) )
					double d_data_t_j = dnorm( data_c[t], mu_c[j], sqrt( sig_c[j] / q_t[t] ), 0 );
...
// ----------- Sort parameters based on pi ------------------------------------|
		...
		sort_sample_t( &mu_c[0], &sig_c[0], &pi_c[0], &k_c );	
